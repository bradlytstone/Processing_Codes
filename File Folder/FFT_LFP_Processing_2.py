...
if laser_check:
    ...
    for l in range(0,len(analyze_tastes)):
        ...
        lag_groups = np.unique(laser_nodes[l].laser_onset_lag[:])
        lag_list = lag_groups.tolist()
        laser_on_trials = []
        laser_off_trials = []
        parsed_trials_on = []
        parsed_trials_off = []
        parsed_trials_on_lag = []
        parsed_trials_off_lag = []
        node_parent = np.mean(lfp_nodes[l],axis=(0))
        
        #Flip through trials and differentiate trials based on duration of lasers
        ...
        for j in range(trial_count):
            ...
            if las_trial_check>0:
                for m in range(0,len(lag_list)):
                    trial_num = j
                    lag_duration = int(lag_list[m])
                    parsed_trials_on_lag.append(node_parent[j])
                vars()[analyze_tastes[l]+"_on_"+str(lag_duration)]= np.asarray(parsed_trials_on_lag)
                trial_num = j
        ...
        vars()["f_"+analyze_tastes[l]+"_on_"+str(lag_duration)], vars()["p_"+analyze_tastes[l]+"_on_"+str(lag_duration)] = periodogram(np.asarray(parsed_trials_on_lag), fs = 1000.0, return_onesided=True)
