function [comb_cond1, comb_cond2, power_spec] = katz_LFP_stone_spec_tastes_2(dirname)
...
for x=1:sessions(1)
    ...
    MeanNormP=zeros(8193,131); %Create array for normalizing power (column length dictated by window/overlap);
    MeanP=zeros(8193,131);     %Create array for mean power (column length dictated by window/overlap);
