...
for x=1:sessions(1)
    ...
    meanfact=size(array,1)*size(taste_data,1); %calculate denominator; trials*channels
   ...
   % Tgood = union(Tgood,find(T > 1.5)); %indicates time to draw into the baseline power calcs
    ...
    switch choice
        ...
        case 'Normalized'
        power_spec = subplot(2,1,x); hold on; imagesc(T(Tgood)-1,F(good),(MeanNormP(good,Tgood)*10));  axis xy; colorbar;     
    end
